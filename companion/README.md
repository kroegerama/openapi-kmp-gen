# companion

Runtime library for code generated by `openapi-kmp-gen`. Provides HTTP client setup, authentication, request/response handling, serialization
utilities, and JWT parsing for Kotlin Multiplatform projects.

## Package Structure

```
com.kroegerama.openapi.kmp.gen.companion
├── ApiHolder.kt              # Abstract base for generated API singletons
├── ApiDefaults.kt            # Default JSON config, HTTP client factory, user agent
├── AuthItem.kt               # Sealed interface for auth types (Basic, Bearer, ApiKey)
├── AuthPlugin.kt             # Ktor plugin that injects auth into requests
├── CallResponse.kt           # Either-based response/error types (Arrow)
├── Request.kt                # eitherRequest extension for HttpClient
├── Serializers.kt            # KSerializer implementations (Base64, epoch, ISO 8601, ImmutableList)
├── SerializerUtils.kt        # Parameter serialization helpers (path, query, header, cookie)
├── FormDataContent.kt        # Helpers to convert @Serializable objects to form data
├── JWT.kt                    # JWT parser (no signature validation)
└── keycloak/
    ├── Keycloak.kt               # Keycloak client: login, auto-refresh, logout, discovery, auth-code and device flows
    ├── KeycloakAuthorization.kt  # Authorization Code + PKCE request building and redirect parsing
    ├── KeycloakDeviceAuthorization.kt # RFC 8628 device grant: user code + verification URI handle
    ├── KeycloakEndpoints.kt      # Realm URL convention + OpenID Connect discovery model
    ├── KeycloakTokens.kt         # Serializable token set with expiry checks
    ├── Pkce.kt                   # PKCE (RFC 7636) S256 verifier/challenge pair
    ├── Sha256.kt                 # internal pure-Kotlin SHA-256 (PKCE only)
    └── KeycloakWebViewClient.kt  # androidMain only: WebViewClient that intercepts the auth-code redirect
```

## Platform Source Sets

| Source Set    | Engine     | User Agent Source                  |
|---------------|------------|------------------------------------|
| `androidMain` | OkHttp     | OkHttp version + Android API level |
| `jvmMain`     | OkHttp     | OkHttp version + OS name/version   |
| `appleMain`   | Darwin     | -                                  |
| `iosMain`     | (inherits) | UIDevice system name/version       |
| `macosMain`   | (inherits) | NSProcessInfo OS version           |
| `linuxMain`   | Curl       | uname sysname/release              |
| `mingwMain`   | WinHttp    | RtlGetVersion Windows version      |

## Key Components

### `ApiHolder`

Abstract base class that generated API objects extend (e.g. `object Api : ApiHolder()`). Manages:

- `baseUrl` - server URL, overridable at runtime
- `json` - kotlinx.serialization `Json` instance (defaults to `createDefaultJson()`)
- `client` - Ktor `HttpClient`, created lazily on first access and reconfigurable via `updateClient()` (cookies, compression, logging with header
  sanitizing, custom decorator); the previous client is closed on replacement
- Auth provider registration via `setAuthProvider(id, provider)` / `clearAuthProvider(id)` - thread-safe (copy-on-write map behind an
  `AtomicReference`), providers registered after client creation are picked up by subsequent requests

### `AuthPlugin`

Ktor `HttpClientPlugin` that intercepts requests and applies auth items. Generated service methods call `authKeys(...)` to declare which auth schemes
a request requires. The plugin resolves each key through registered `AuthItemProvider` lambdas.

Supported auth types (`AuthItem`):

- `Basic(username, password)`
- `Bearer(token)`
- `ApiKey(position, name, value)` - position: `Header`, `Query`, or `Cookie`

### `CallResponse` / `Request`

`eitherRequest<T>` is an `HttpClient` extension that wraps Ktor requests into `Either<CallException, HttpCallResponse<T>>` (Arrow). Error hierarchy:

| Type                         | Cause                        |
|------------------------------|------------------------------|
| `HttpCallException`          | Non-success HTTP status      |
| `CallSerializationException` | Body deserialization failure |
| `IOCallException`            | Network / IO error           |
| `UnexpectedCallException`    | Any other throwable          |

`typed<E>()` converts `EitherCallResponse<T>` to `EitherTypedCallResponse<E, T>`, deserializing the error body into type `E` as
`TypedHttpCallException<E>`. If the error body cannot be decoded as `E`, the original `HttpCallException` is kept.

### Serializers

Custom `KSerializer` implementations and type aliases for generated models:

| Type Alias                      | Serializer                    | Format                     |
|---------------------------------|-------------------------------|----------------------------|
| `SerializableBase64`            | `Base64Serializer`            | Base64 string ↔ ByteArray  |
| `SerializableEpochSeconds`      | `EpochSecondsSerializer`      | Long (seconds) ↔ Instant   |
| `SerializableEpochMilliseconds` | `EpochMillisecondsSerializer` | Long (millis) ↔ Instant    |
| `SerializableISO8601Instant`    | `ISO8601InstantSerializer`    | ISO 8601 string ↔ Instant  |
| `SerializableImmutableList<T>`  | `ImmutableListSerializer`     | JSON array ↔ ImmutableList |

Parameter serialization helpers for generated service methods:

- `appendSerializedPathSegment` / `createSerializedPathSegment`
- `appendSerializedQueryParameter`
- `appendSerializedHeaderParameter`
- `appendSerializedCookieParameter`

Each helper has a reified variant and an overload taking an explicit `serializer`. The explicit
overload is required for the annotated type aliases above: reified serializer lookup only sees the
underlying type (`Instant`, `ByteArray`) and would fall back to its builtin serializer.
`encodeNullableToJsonElement` covers the same case for request bodies, and the `eitherRequest`
overload with an explicit `deserializer` covers response bodies.

### `FormDataContent`

Extension functions to convert any `@Serializable` object to:

- `asFormDataContent()` - URL-encoded form data
- `asMultiPartFormDataContent()` - multipart form data

### `JWT`

Parses JWT tokens to extract header, payload, and claims. Does **not** validate signatures.

- `JWT.parse(token)` / `JWT.parseOrNull(token)`
- Standard claims: `issuer`, `subject`, `audience`, `expiresAt`, `notBefore`, `issuedAt`, `id`
- Custom claims: `getClaim(name)`
- Time checks: `isExpired(leeway)`, `isTimeValid(leeway)`

### `Keycloak`

Self-contained Keycloak / OpenID Connect client (`keycloak` subpackage) that obtains, stores, and refreshes OAuth 2.0 tokens. Uses its own lightweight
`HttpClient`, never the `ApiHolder` client, so token requests bypass the API's auth plugin.

- Grants: `login(username, password)` (Direct Access Grants), `loginClientCredentials()`, the Authorization Code + PKCE flow, and the Device
  Authorization Grant (RFC 8628)
- `bearerOrNull()` refreshes the access token automatically when expired; `asBearerProvider()` adapts it to a generated bearer `Auth` variant
- Token persistence is delegated to the app via `KeycloakTokenLoader` / `KeycloakTokenListener`; state is observable via
  `tokens: StateFlow<KeycloakTokens?>`, `isLoggedIn: Flow<Boolean>`, and `sessionEnded: SharedFlow<KeycloakSessionEndReason>`
- Logout both ways: backchannel `logout()` and OIDC RP-Initiated (browser) logout; `userInfo()` fetches the end-user's claims
- Errors use the module's `Either<CallException, T>` model; `CallException.keycloakErrorOrNull()` extracts the Keycloak error body

```kotlin
val keycloak = Keycloak(
    baseUrl = Url("https://auth.example.com"),
    realm = "my-realm",
    clientId = "my-app",
    tokenLoader = { storage.loadTokens() },
    tokenListener = { storage.saveTokens(it) },
)

keycloak.login("alice", "password")

// OpenAPI oauth2/openIdConnect security schemes generate bearer Auth variants:
Api.setAuthProvider(Auth.OIDCAuth(keycloak.asBearerProvider()))
```

The [Keycloak guide](docs/keycloak.md) covers the full walkthrough: refresh and persistence semantics, the browser-based Authorization Code + PKCE
flow (with per-platform recipes, e.g. [Custom Tabs on Android](docs/keycloak-android-custom-tabs.md)), the Device Authorization Grant, RP-initiated
logout, and the [integration test setup](keycloak-testenv/README.md).

## Dependencies

All dependencies are exposed with `api` scope.

| Dependency                        | Source Set  |
|-----------------------------------|-------------|
| `kotlinx-coroutines-core`         | common      |
| `kotlinx-datetime`                | common      |
| `kotlinx-collections-immutable`   | common      |
| `kotlinx-serialization-json`      | common      |
| `ktor-client-core`                | common      |
| `ktor-client-content-negotiation` | common      |
| `ktor-client-encoding`            | common      |
| `ktor-client-auth`                | common      |
| `ktor-client-logging`             | common      |
| `ktor-serialization-kotlinx-json` | common      |
| `arrow-core`                      | common      |
| `compose-runtime-annotation`      | common      |
| `ktor-client-okhttp`              | Android/JVM |
| `kotlinx-coroutines-android`      | Android     |
| `ktor-client-darwin`              | Apple       |
| `ktor-client-curl`                | Linux       |
| `ktor-client-winhttp`             | MinGW       |

## Usage in Generated Code

The `generated` module demonstrates how the companion library is consumed. The code generator produces:

1. An `Api` object extending `ApiHolder` with server URLs and auth helpers
2. `Auth` sealed interface variants mapping OpenAPI security schemes to `AuthItem` types
3. Service objects (e.g. `DefaultApi`) using `eitherRequest`, `authKeys`, `createSerializedPathSegment`, and `appendSerializedQueryParameter`
4. Model data classes using `SerializableBase64`, `@Immutable`, etc.

```kotlin
// Setup
Api.setAuthProvider(Auth.BearerAuth {
    AuthItem.Bearer("my-token")
})

// Call
val result = DefaultApi.getPhoto(1)
// result: Either<CallException, HttpCallResponse<Photo>>
```

## Notes

- Targets: JVM (11+), Android (minSdk 21), iOS, macOS, Linux, MinGW
- Explicit API mode enabled
- Published via Vanniktech Maven Publish plugin
- Includes consumer ProGuard rules for Android
- Build config is auto-generated with Ktor/Compose/companion version strings
