...
import com.kroegerama.openapi.kmp.gen.language.asConstantName
import com.kroegerama.openapi.kmp.gen.language.asFieldName
import com.kroegerama.openapi.kmp.gen.language.asFunctionName
...
class SpecConverter(
    ...
    fun convert(): SpecModel {
        ...
        val fileHeader = with(info) {
            buildString {
                ...
                appendLine()
                val formattedDateTime = createdAt.format(DateTimeFormatter.RFC_1123_DATE_TIME)
    ...
    private fun convertParameter(parameter: Parameter): SpecParameter {
        return SpecParameter(
            ...
            style = parameter.style ?: getDefaultParameterStyle(parameter),
            explode = parameter.explode ?: getDefaultParameterExplode(parameter)
        )
    }
    ...
    private fun getDefaultParameterStyle(parameter: Parameter): Parameter.StyleEnum = when (parameter) {
        is CookieParameter -> Parameter.StyleEnum.FORM
        is HeaderParameter -> Parameter.StyleEnum.SIMPLE
        is PathParameter -> Parameter.StyleEnum.SIMPLE
        is QueryParameter -> Parameter.StyleEnum.FORM
        else -> throw IllegalStateException("Invalid parameter type $parameter")
    }

    private fun getDefaultParameterExplode(parameter: Parameter): Boolean = when (parameter) {
        is CookieParameter -> true
        is HeaderParameter -> false
        is PathParameter -> false
    ...
    private fun convertEnumConstants(enum: List<*>): List<SpecSchema.Enum.Constant> {
        return enum.map {
            SpecSchema.Enum.Constant(
                value = it.toString(),
                name = it.toString().asConstantName()
            )
        }
    }
